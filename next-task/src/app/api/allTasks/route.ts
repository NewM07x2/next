...
export async function GET() {
  // console.log('next/next-task/src/app/api/allTasks/route.ts: GET')
  try {
    const sql = `
      SELECT
        id,
...
export async function POST(request: Request) {
  try {
    ...
    switch (action) {
      ...
      case 'insert':
        const insertSql = `
        INSERT INTO task (task_name, task_detail) 
        VALUES ($1, $2)
        ;`
        const insertValues = [task_name, task_detail];
        result = await executeSql(insertSql, insertValues);
        break;
      default:
